...
final class PostgreSQLNotesRepositorySpec extends FunSpecWithFixtures with Matchers with ScalaFutures with OptionValues {
  ...
  describe("Notes Repository") {
    ...
    val row1 = Note(none, "docker ps", "list docker process", List(Tag(TagId(1).some, TagName("docker"))))
    val row2 = Note(none, "docker ps", "list docker processesses", List(Tag(TagId(1).some, TagName("docker"))))
    ...
    it("should add a note and return an int representing rows affected when non duplicate terms are passed"){ flyway =>
      ...
      repo.insertNote(row2.copy(term="docker ps -a")).unsafeRunSync shouldEqual 1
    }
